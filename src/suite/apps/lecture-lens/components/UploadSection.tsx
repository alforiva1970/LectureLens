...
export function UploadSection({
  ...
  fileInputRef, handleFileChange, handleProcessQueue, setFile, resetUpload,
  useThreePass, setUseThreePass, error, videoUrl
}: UploadSectionProps) {
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold uppercase tracking-wider text-black/40 dark:text-white/40">2. Configura Analisi</h3>
        </div>
        
        <button
          onClick={() => setUseThreePass(!useThreePass)}
          className={cn(
            "w-full p-5 rounded-[32px] border transition-all flex items-center gap-4 group relative overflow-hidden",
            useThreePass 
              ? "bg-emerald-500/10 border-emerald-500/50 text-emerald-700 dark:text-emerald-400 shadow-lg shadow-emerald-500/10" 
              : "bg-white dark:bg-zinc-900 border-black/10 dark:border-white/10 hover:border-black/20 dark:hover:border-white/20"
          )}
        >
          <div className={cn(
            "w-12 h-12 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110",
            useThreePass ? "bg-emerald-500 text-white shadow-lg shadow-emerald-500/20" : "bg-black/5 dark:bg-white/5 text-black/40 dark:text-white/40"
          )}>
            <Sparkles className={cn("w-6 h-6", useThreePass ? "animate-pulse" : "")} />
          </div>
          <div className="flex-1 text-left">
            <div className="flex items-center gap-2">
              <span className="font-bold text-sm">Analisi Tripla (Modalità Pro)</span>
              {useThreePass && <span className="text-[10px] bg-emerald-500 text-white px-1.5 py-0.5 rounded-md uppercase font-black">Attiva</span>}
            </div>
            <p className="text-xs text-black/50 dark:text-white/50 mt-0.5 leading-tight">
              Esegue 3 passaggi separati con Gemini Pro per la massima precisione. Consigliata per esami difficili.
            </p>
          </div>
          <div className={cn(
            "w-10 h-6 rounded-full relative transition-colors",
            useThreePass ? "bg-emerald-500" : "bg-black/10 dark:bg-white/10"
          )}>
            <div className={cn(
              "absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm",
              useThreePass ? "left-5" : "left-1"
            )} />
          </div>
        </button>
      </section>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold uppercase tracking-wider text-black/40 dark:text-white/40">3. Carica Video</h3>
        </div>
        
        {videoUrl && (
          <div className="mb-6 rounded-2xl overflow-hidden border border-black/10 dark:border-white/10 bg-black">
            <video 
              src={videoUrl} 
              controls 
      ...
      <button
        ...
        className={cn(
          ...
          queue.length === 0 || isProcessingQueue 
            ...
            : useThreePass
              ? "bg-gradient-to-r from-emerald-600 to-teal-600 text-white hover:from-emerald-700 hover:to-teal-700 shadow-xl shadow-emerald-500/20"
      ...
      >
        {isProcessingQueue ? (
          <>
            ...
            <span>{useThreePass ? "Analisi Tripla in corso..." : "Elaborazione in corso..."}</span>
          </>
        ) : (
          <>
            <Sparkles className={cn("w-5 h-5", useThreePass ? "animate-pulse" : "")} />
            <span>
              {queue.length > 1 
                ? (useThreePass ? "Analisi Tripla Coda" : "Analizza Coda (Notturno)") 
                : (useThreePass ? "Analisi Tripla" : "Analizza Lezione")}
            </span>
          </>
        )}
      </button>
